...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createStartup() {
    ...
    const body = { ...req.body };
    if (body?.connection_request?.length > 0) {
      const create_connection_request = body.connection_request;
      body.connection_request = {
        create: create_connection_request,
      };
    } else {
      delete body.connection_request;
    }
